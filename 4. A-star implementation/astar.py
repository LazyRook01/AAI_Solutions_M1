...
class PuzzleState:
    def __init__(self, state, parent=None, action=None):
        ...
        self.g = 0  # Cost from the initial state to this state (initialized to 0)
    ...
    def f(self):
        # The cost function for A* is the sum of the actual cost (g) and the heuristic value (h)
...
def astar(initial_state, goal_state):
    ...
    heapq.heappush(priority_queue, (initial_state.f(), initial_state))
    ...
    while priority_queue:
        ...
        for successor in successors:
            if successor not in visited:  # If the successor state hasn't been visited
                successor.g = current_state.g + 1  # Increment the cost (g) from the initial state
                heapq.heappush(priority_queue, (successor.f(), successor))  # Add the successor to the priority queue for exploration
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print("Usage: python astar.py filename.txt")  # Print usage instructions if the command line argument is missing or incorrect
    ...
    solution = astar(PuzzleState(initial_state), goal_state)  # Solve the puzzle using A*
    if solution:
